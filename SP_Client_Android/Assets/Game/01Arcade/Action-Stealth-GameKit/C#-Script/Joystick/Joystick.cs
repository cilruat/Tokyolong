...
public class Joystick : MonoBehaviour {
	...
	private float lastFingerId = -1;					// Finger last used for this joystick
	...
	void ResetJoystick (){
		...
		if ( touchPad )
			gui.color = new Color(gui.color.r,
			                      gui.color.g,
			                      gui.color.b,
			                      0.025f);
	}
	...
	void Update (){	
			...
			float speed = 2.5f;
			target.transform.position += new Vector3 (position.x * Time.deltaTime * speed,
				0,
				position.y * Time.deltaTime * speed);

			//Limiters
			if (target.transform.position.x > 9) {
				target.transform.position = new Vector3 (9,
					target.transform.position.y,
					target.transform.position.z);
			} else if (target.transform.position.x < -9) {
				target.transform.position = new Vector3 (-9,
					target.transform.position.y,
					target.transform.position.z);
			} else if (target.transform.position.z > 5) {
