	...
	public class WAMGameController : MonoBehaviour 
	{
		...
		void ShowTargets( int targetCount )
		{
			...
			while ( targetCount > 0 && maximumTries > 0 )
			{
                ...
                if (!spawnPositions[randomPosition].GetComponentInChildren<WAMMole>())
                {
                    ...
                    Transform newTarget = Instantiate(moles[randomTarget], spawnPositions[randomPosition].position, Quaternion.identity);
		...
		void HitBonus( Transform hitSource )
		{
			...
			if ( bonusEffect )
			{
                ...
                if (hitSource.GetComponent<WAMMole>().bonusMultiplier >= 0) newBonusEffect.Find("Text").GetComponent<Text>().text = "+" + (hitTargetBonus * streak * hitSource.GetComponent<WAMMole>().bonusMultiplier).ToString();
                else newBonusEffect.Find("Text").GetComponent<Text>().text = (hitTargetBonus * streak * hitSource.GetComponent<WAMMole>().bonusMultiplier).ToString();
			...
			ChangeScore(hitTargetBonus * streak * hitSource.GetComponent<WAMMole>().bonusMultiplier);
		}
