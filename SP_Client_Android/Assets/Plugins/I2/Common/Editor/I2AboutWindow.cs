...
namespace I2
{
    ...
    public class I2AboutHelper
    {
        ...
        static public void StartCheckingForUpdates()
        {
        ...
        static void CheckIfAnyPluginNeedsUpgrading()
        {
            ...
            //    var pluginData = I2AboutWindow.PluginsData.Plugins[i];

            //    if ((ShouldUpgrade && I2AboutWindow.bNotifyOfNewMainVersions) || (HasNewBeta && I2AboutWindow.bNotifyOfNewBetas))
            //    {
            //        openWindow = true;
            //        I2AboutWindow.DoSkipVersion(pluginData.Name, pluginData.AssetStoreVersion, pluginData.BetaVersion);
            //    }
            //}
    ...
    public class I2AboutWindow : EditorWindow
    {
        ...
        public static void DoShowScreen()
        {
            ...
            PluginsData = null;
            EditorWindow.GetWindowWithRect<I2AboutWindow>(new Rect(0f, 0f, 750f, 680f), true, "Plugins Manager");
        }
        ...
        static void LoadDefaultData()
        {
            ...
            PluginsData.Plugins = new PluginData[3];
            PluginsData.Plugins[0] = new PluginData()
            {
                Name = "TextAnimation",
                Description = "Pixel Perfect Styles and Animations for TEXT and IMAGES",
                TrailerURL = "https://www.youtube.com/channel/UCBYYngAJJjCuoZhu1UbwoAw",
                DocumentationURL = "http://inter-illusion.com/assets/I2TextAnimationManual/TextAnimation.html",
                TutorialsURL = "http://inter-illusion.com/assets/I2TextAnimationManual/TextAnimation.html",
                ForumURL = "http://www.inter-illusion.com/forum/i2-TextAnimation",
                AssetStoreURL = "https://www.assetstore.unity3d.com/en/#!/publisher/5916",
                AssetStoreVersion = "none",
                BetaVersion = "1.0.0 b3"
            };
            ...
            PluginsData.Plugins[2] = new PluginData()
            {
                Name = "I2 MiniGames",
        ...
        public void OnGUI()
        {
            ...
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("Never show this again", EditorStyles.toolbarButton))
            {
                bNotifyOfNewMainVersions = false;
                bNotifyOfNewBetas = false;
                this.Close();
            }
            GUILayout.Space(5);
        ...
        void OnGUI_PluginVersion(PluginData pluginData)
        {
            ...
            if (HasNewBeta || HasNewMainVersion)
            {
                ...
                {
                    DoSkipVersion(pluginData.Name, pluginData.AssetStoreVersion, pluginData.BetaVersion);
                }
        ...
        public static void DoSkipVersion(string PluginName, string MainVersion, string BetaVersion)
        {
            EditorPrefs.SetString("I2 SkipVersion " + PluginName, MainVersion + ";" + BetaVersion);
        }
        ...
        {
            if (I2.I2Analytics.PluginsVersion.TryGetValue(pluginData.Name, out InstalledVersion))
            {
                ShouldUpgrade = !string.IsNullOrEmpty(pluginData.AssetStoreVersion) && (pluginData.AssetStoreVersion != "none") && (IsNewerVersion(pluginData.AssetStoreVersion, InstalledVersion));
                HasNewBeta = !string.IsNullOrEmpty(pluginData.BetaVersion) && (pluginData.BetaVersion != "none") && (IsNewerVersion(pluginData.BetaVersion, InstalledVersion));

                string skipVersion = EditorPrefs.GetString("I2 SkipVersion " + pluginData.Name, "");
        ...
        static char[] mVersionSeparators = " .".ToCharArray();
        public static bool IsNewerVersion(string newVersion, string oldVersion)
        {
            var oV = oldVersion.Split(mVersionSeparators);
            var oN = newVersion.Split(mVersionSeparators);

            for (int i=0; i<oV.Length; ++i)
            {
                if (oN.Length <= i)     // "1.0" is newer than "1.0.1"
                    return true;

                if (oN[i].Length > oV[i].Length)
                    return true;
                else
                if (oN[i].Length < oV[i].Length)
                    return false;

                int val = oN[i].CompareTo(oV[i]);
