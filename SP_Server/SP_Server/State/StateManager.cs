...
public class StateManager<T, T2>
{
    ...
    // 메시지 관리.
    //----------------------------------------------
    Dictionary<IState, MessageDispatcher<T, T2>> message_dispatcher;


    public StateManager()
    {
        this.message_dispatcher = new Dictionary<IState, MessageDispatcher<T, T2>>();
    }
    ...
    public void register_message_handler(IState state, Enum key, MessageHandlerDelegate<T, T2> fn)
    {
        ...
        {
            this.message_dispatcher.Add(state, new MessageDispatcher<T, T2>());
        }
