...
namespace SgLib
{
    public class RatingRequester : MonoBehaviour
    {
        ...
        void GameManager_GameStateChanged(GameEvent newGameEvent)
        {
            if (newGameEvent == GameEvent.Start)
            {
                // A new game has started.
                SetGamesPlayed(GetGamesPlayed() + 1);
            }
            else if (newGameEvent == GameEvent.GameOver)
            {
                if (CanRequestNow())
                    StartCoroutine(MakeRatingRequest(3));
            }
        }
